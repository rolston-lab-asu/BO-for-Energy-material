"""
NIMO Bayesian optimization loop for binary oxide (M-O) candidates.

The Materials Project API acts as the oracle — no robot needed.
Goal: find the most stable (lowest formation_energy_per_atom) binary oxide
...
Features:
  ...
  - All parameters driven by binary_oxide_config.yaml

Usage:
    export MP_API_KEY="your_key_here"
    python mp_nimo_loop.py                       # PHYSBO (default from config)
    python mp_nimo_loop.py --method BLOX
    python mp_nimo_loop.py --method RE           # random baseline
    python mp_nimo_loop.py --config my.yaml      # custom config

Prerequisites:
    Run mp_fetch_candidates.py first to generate the candidates CSV.
"""
...
def main(method, config_path):
    ...
    for cycle in range(start_cycle, NUM_CYCLES):
        ...
        for idx, cand in proposed:
            ...
            print(f"  Querying: {formula} ({mid})  [{metal}-O]")
    ...
    if len(best_curve) > 1:
        ...
        axes[0].set_title(f"NIMO [{method}] — Binary oxides M-O")
        axes[0].legend()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="binary_oxide_config.yaml")
