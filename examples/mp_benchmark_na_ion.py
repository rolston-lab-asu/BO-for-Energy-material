"""
Benchmark NIMO methods (RE, PHYSBO, BLOX, NTS, AX) on the Na-ion intercalation
cathode candidate pool with TWO simultaneous objectives.

No MP API calls — both objectives are read directly from na_ion_candidates.csv,
which must be fully populated before running.

Objectives (both minimized by NIMO):
  1. neg_avg_voltage  = -min(average_voltage, practical_max)  → maximise voltage
  2. max_delta_volume                                          → minimise volume change

Practical voltage cap: 4.0 V (Na-ion electrolyte stability limit).
Without capping, NIMO chases unphysical high-voltage outliers.

Pool: 206 Na-ion cathode frameworks from the Materials Project.
Coverage: 80 cycles = 39%, 160 cycles = 78%.  Discovery is meaningful at
this coverage (unlike Li-ion at 9%).

Metrics reported per method per seed:
  - best_voltage      : max capped average_voltage seen so far at each cycle
  - best_delta_volume : min max_delta_volume seen so far at each cycle
  - hypervolume       : 2D hypervolume of the current Pareto front
  - discovery         : cycle (1-indexed) when global_best_voltage first reached
                        (within 1e-4 V tolerance), or None

Summary table includes:
  - mean/std discovery cycle and found%
  - mean final hypervolume
  - Wilcoxon rank-sum p-values vs RE baseline (two-sided Mann-Whitney U)

Usage:
    python mp_benchmark_na_ion.py [--config na_ion_config.yaml]
    python mp_benchmark_na_ion.py --seeds 5 --cycles 80
    python mp_benchmark_na_ion.py --methods PHYSBO BLOX RE --seeds 3

Prerequisites:
    na_ion_candidates.csv must have average_voltage AND max_delta_volume
    filled for ALL rows.
"""
...
def hypervolume_2d(points, ref):
    ...
    Both objectives minimised. ref must be strictly worse than all Pareto points.
    """
    ref_v, ref_m = ref
...
def run_one(method, candidates_full, feature_cols, cfg,
    ...
    Returns
    -------
    volt_curve : list[float]  best capped voltage after each cycle
    mdv_curve  : list[float]  best delta_volume after each cycle
...
def significance_vs_re(all_results, methods):
    """
    Two-sided Mann-Whitney U test comparing final-HV distributions of each
    method against RE.  Returns dict: method → (statistic, p_value).
    Mann-Whitney U is appropriate here because seeds are independent runs
    (not paired), and HV distributions may not be Gaussian.
    """
...
def plot_results(all_results, methods, cfg, output_prefix, num_seeds):
    NUM_CYCLES  = cfg["nimo"]["num_cycles"]
    SEED_CYCLES = cfg["nimo"]["seed_cycles"]
    PRACTICAL_MAX = cfg["battery"]["voltage_practical_max"]
    ...
    ax1.axvline(SEED_CYCLES, color="gray", linestyle="--", alpha=0.5,
                label="RE → AI switch")
    ax1.axhline(PRACTICAL_MAX, color="red", linestyle=":", alpha=0.4,
                label=f"Practical cap ({PRACTICAL_MAX} V)")
    ax1.set_xlabel("Cycle")
    ax1.set_ylabel("Best avg voltage (V vs Na/Na⁺)")
    ax1.set_title(f"Na-ion  —  Voltage convergence  (mean ± 1σ, {num_seeds} seeds)")
    ...
    ax2.set_title(f"Na-ion  —  2D hypervolume  (mean ± 1σ, {num_seeds} seeds)")
    ax2.legend(fontsize=8)
    ax2.grid(True, alpha=0.3)

    # Panel 3: discovery cycle bar chart
    ax3 = axes[2]
    disc_means, disc_errs, labels = [], [], []
    for method in methods:
    ...
    ax3.set_title("Discovery cycle (lower = better;\n>num_cycles = never found)")
    ax3.legend(fontsize=8)
    ax3.grid(True, alpha=0.3, axis="y")

    fig.tight_layout()
    out_path = f"{output_prefix}_benchmark_Na.png"
...
def main(methods, config_path, num_seeds, num_cycles_override, practical_max_override=None):
    ...
    if not os.path.exists(CANDIDATES_FILE):
        print(f"Error: {CANDIDATES_FILE} not found. Run mp_fetch_battery.py "
              "with working_ion=Na first.")
        sys.exit(1)
    ...
    best_v_cand = next(c for c in known
                       if min(float(c["average_voltage"]), PRACTICAL_MAX)
                       >= global_best_voltage - 1e-5)
    best_m_cand = next(c for c in known
                       if abs(float(c["max_delta_volume"]) - global_best_mdv) < 1e-5)

    # Reference point in (neg_volt, mdv) space — slightly worse than worst oracle
    max_neg_volt = max(-min(v, PRACTICAL_MAX) for v in oracle_volts)
    max_mdv      = max(oracle_mdvs)
    ref_point    = (max_neg_volt + 0.1, max_mdv + 0.01)

    pool_size = len(candidates_full)
    num_cycles = cfg["nimo"]["num_cycles"]
    coverage   = 100.0 * num_cycles / pool_size

    print(f"Candidates          : {pool_size} total, {len(known)} measured")
    print(f"Practical voltage cap: {PRACTICAL_MAX} V (Na-ion electrolyte limit)")
    print(f"Coverage            : {num_cycles}/{pool_size} = {coverage:.0f}%")
    print(f"Global best voltage : {global_best_voltage:.4f} V  "
          f"({best_v_cand.get('framework_formula','?')}, "
          f"{best_v_cand.get('battery_id','?')})")
    print(f"Global best ΔV      : {global_best_mdv:.6f}  "
          f"({best_m_cand.get('framework_formula','?')}, "
          f"{best_m_cand.get('battery_id','?')})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark NIMO methods on Na-ion cathode candidates — "
            "2-objective (voltage + volume change), no API calls."
        )
    )
    parser.add_argument(
        "--methods", nargs="+",
        default=["RE", "PHYSBO", "BLOX"],
        choices=["RE", "PHYSBO", "BLOX", "NTS", "AX"],
        help="Methods to benchmark (default: RE PHYSBO BLOX)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--config", default="na_ion_config.yaml",
        help="Path to YAML config (default: na_ion_config.yaml)",
    )
    parser.add_argument(
        "--practical-max", type=float, default=None,
        help="Override voltage_practical_max from config (e.g. 5.0 to remove cap)",
    )
